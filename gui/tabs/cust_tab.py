...
from controller import cust_ctrl
from tkinter import messagebox
from uuid import uuid4
import helper


class CustomerTabView(CTkTabview):
    ...
    def __init__(self, master):
        ...
        self.search = CTkEntry(master=table_tab, width=305, placeholder_text="Search Customer...", border_color="#99BC85", border_width=2)
        self.search.bind("<KeyRelease>", self.search_callback)
        self.search.pack(side="left", anchor="w", padx=5, pady=5)

        # Initialize Create Tab
        # First Row
        CTkLabel(master=create_tab, text="Customer:").grid(sticky="w", row=0, column=0, padx=5, pady=5)
        ...
        CTkLabel(master=create_tab, text="Balance:").grid(sticky="w", row=3, column=0, padx=5)
        self.balance_entry = CTkEntry(master=create_tab, width=145, border_color="#2A8C55", border_width=2)
        self.balance_entry.grid(sticky="w", row=3, column=1, padx=5)

        # Add Button
        self.add_supplier_button = CTkButton(master=create_tab, text="Submit", fg_color="green", hover_color="#207244",
                                             font=("Arial Bold", 15), width=145, command=self.add_customer)
        self.add_supplier_button.grid(sticky="w", row=3, column=5, padx=5, pady=5)
    ...
    def add_customer(self):
        entries = {
            "name": self.name_entry.get(), "address": self.address_entry.get(), "tin_no": self.tin_entry.get(), "contactp": self.contactp_entry.get(),
            "contactno": self.contactno_entry.get(), "terms": self.terms_entry.get(), "balance": self.balance_entry.get()
        }
        ...
        if "" == entries.get("name"):
            ...
            contact_p, contact_no, terms, balance = entries.get("contact_p"), entries.get("contact_no"), entries.get("terms"), float(entries.get("balance"))
            iid = str(uuid4())[:6]
            values = [name, address, tin_no, terms, balance]

            cust_ctrl.add_customer(
                iid=iid,
                name=name,
                address=address,
                tin_no=tin_no,
            ...
            # Update All Rows
            self.all_rows = helper.return_updated_customers()
            self.clear_entries()
